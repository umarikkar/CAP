...
from dataset.chammiv1 import get_chammiv1_linear_dataloaders
from dataset.jumpcp import get_jumpcp_linear_dataloaders
from dataset.so2sat import get_so2sat_linear_dataloaders
from dataset.dataset_utils import self_normalize

from models.linear_model_v2 import get_linear_model
...
class LinearTrainer(Trainer):
    ...
    def __init__(self, cfg: DictConfig) -> None:
        ...
        # actually do not initialize the parent classes. Just borrow their functions. 
        self.cfg = cfg
    ...
    def _initialize_seed_and_jobid(self) -> tuple[str, str, int]:
        ...
        if self.cfg.logging.scc_jobid is None or self.cfg.logging.scc_jobid=='':
            jobid = f"{self.cfg.train.encoder_cpt}_{self.cfg.data.training_dataset}_{self.cfg.model.aggregator.name}"
            self.cfg.logging.scc_jobid = jobid
        else:
            jobid = f"{self.cfg.train.encoder_cpt}_{self.cfg.data.training_dataset}_{self.cfg.model.aggregator.name}_{self.cfg.logging.scc_jobid}"
            self.cfg.logging.scc_jobid = jobid
    ...
    def prepare_dataloaders(self) -> tuple[DataLoader, Optional[DataLoader], dict[str, DataLoader]]:
        ...
        if self.training_dataset == "chammiv1":
            ...
            train_loader, _, test_loaders = get_chammiv1_linear_dataloaders(
                **chammiv1_cfg,
                features_path=self.cfg.train.features_path,
            ...
            train_loader, valid_loader, test_loaders = get_jumpcp_linear_dataloaders(
                **jumpcp_cfg,
                features_path=self.cfg.train.features_path,
                batch_size=batch_size,
                eval_batch_size=eval_batch_size,
            ...
            train_loader, valid_loader, test_loaders = get_so2sat_linear_dataloaders(
                **so2sat_cfg,
                features_path=self.cfg.train.features_path,
                in_type=self.cfg.model.aggregator.chan_setting.split('_')[0],
                batch_size=batch_size,
                eval_batch_size=eval_batch_size,
                num_workers=num_workers,
            )
    ...
    def prepare_models(self) -> torch.nn.Module:
        ...
        model = get_linear_model(**model_cfg)

        if 'preloaded' in self.cfg.model.name:
            ...
            loaded_cpt = {k.replace('blocks.11.attn.', ''):v for k,v in loaded_cpt.items() if "blocks.11.attn." in k}

            msg = model.aggregator.blocks[0].attn.load_state_dict(loaded_cpt, strict=False)

            print("LOADING ATTENTION BIT IN AGGREGATOR WITH MESSAGE:", msg)
        ...
        for epoch in range(first_epoch + 1, total_epochs + 1):
            ...
            for batch_i, batch in enumerate(self.train_loader):
                ...
                with accelerator.accumulate(self.model):
                    ...
                    if batch_i == 0 and self.cfg.model.aggregator=='CMXA':
                        print(f"ALPHA_GRAD_VALUES: {self.model.aggregator.blocks[0].alpha_chan.item()}, {self.model.aggregator.blocks[0].alpha_chan.grad.item()}")
