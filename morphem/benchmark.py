...
def run_benchmark(
    ...
    if dataset and dataset in ["Allen", "HPA", "CP"]:
        task_dict = task_dict[task_dict["dataset"] == dataset]
    ...
    for idx, row in task_dict.iterrows():
        ...
        result_temp = pd.DataFrame(
            {
                "dataset": [dataset for i in range(len(results["tasks"]))],
                "task": results["tasks"],
                "classifier": [classifier for i in range(len(results["tasks"]))],
